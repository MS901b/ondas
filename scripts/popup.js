...
if (typeof Prototype != 'object') {
    ...
    Event.observe(window, 'load', function() {
        Popups.each(function(item) {
            var pai = $(item.key);
            if (!pai) {
                alert('Não foi possível encontrar o link \'' + item.key + '\' na página para gerar o popup.\n O script foi cancelado.');
                throw $break;
            }
            config = item.value;
        ...
        Aviso.each(function(item) {
            var pai = $(item.key);
            if (!pai) {
                alert('Não foi possível encontrar o link \'' + item.key + '\' na página para gerar o popup no link.\n O script foi cancelado.');
                throw $break;
            }
            config = item.value;
        ...
        Perguntas.each(function(item) {
            var pai = $(item.key);
            if (!pai) {
                alert('Não foi possível encontrar o link \'' + item.key + '\' na página para gerar o popup com pergunta.\n O script foi cancelado.');
                throw $break;
            }
            config = item.value;
        ...
        CarregarGrafico.each(function(item) {
            var pai = $(item.key);
            if (!pai) {
                alert('Não foi possível encontrar o link \'' + item.key + '\' na página para gerar o popup com pergunta.\n O script foi cancelado.');
                throw $break;
    ...
    var Popup = Class.create({
        ...
        abre: function(ev) {
            ...
            switch (this.layout[1]) {
                case 'central':
                    if (seta == "seta_direita") {
                        top -= this.altura / 2 - this.pai.getHeight() / 2;
                        seta_top = this.altura / 2 - 12;
                    }
                    if (seta == "seta_baixo") {
                        left -= this.largura / 2 - this.pai.getWidth() / 2;
                        seta_left = this.largura / 2 - 12;
                    }
                    if (seta == "seta_cima") {
                        left -= this.largura / 2 - this.pai.getWidth() / 2;
                        seta_left = this.largura / 2 - 12;
                ...
                case 'direita':
                    if (seta == "seta_baixo") {
                        left -= this.largura - 30 - this.pai.getWidth() / 2;
                        seta_left = this.largura - 40;
                    }
                    if (seta == "seta_cima") {
                        left -= this.largura - 30 - this.pai.getWidth() / 2;
                        seta_left = this.largura - 40;
                ...
                case 'topo':
                    if (seta == "seta_direita") {
                        top -= 43 - this.pai.getHeight() / 2;
                        seta_top = 30;
                    }
                    break;
                case 'baixo':
                    if (seta == "seta_direita") {
                        top -= this.altura - 30 - this.pai.getHeight() / 2;
                        seta_top = this.altura - 40;
                    }
            }
    ...
    var PopupCallback = Class.create(PopupClick, {
        ...
        initialize: function($super, pai, texto, seta, largura, callb, itens, tudo) {
            if (typeof callb != 'function') {
                alert('A classe PopupCallback precisa receber uma função no seu 5° parâmetro de inicialização;')
                return false;
            }
            this.callb = callb;
            this.itens = itens;
